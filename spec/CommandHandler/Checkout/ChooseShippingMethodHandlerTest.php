...
final class ChooseShippingMethodHandlerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new ChooseShippingMethodHandler($this->orderRepositoryMock, $this->shippingMethodRepositoryMock, $this->shipmentRepositoryMock, $this->eligibilityCheckerMock, $this->stateMachineMock);
    }
    ...
    public function testAssignsChoosenShippingMethodToSpecifiedShipment(): void
    {
        ...
        $cartMock->method('getShipments')->willReturn(new ArrayCollection([$shipmentMock]));
        $cartMock->method('getId')->willReturn('111');
        ...
        self::assertSame($cartMock, $this->handler->__invoke($chooseShippingMethod));
    }
    ...
    public function testThrowsAnExceptionIfOrderCannotHaveShippingSelected(): void
    {
        ...
        $this->orderRepositoryMock->expects(self::once())->method('findOneBy')->with(['tokenValue' => 'ORDERTOKEN'])->willReturn($cartMock);
        $this->shippingMethodRepositoryMock->method('findOneBy')->with(['code' => 'DHL_SHIPPING_METHOD'])->willReturn(null);
