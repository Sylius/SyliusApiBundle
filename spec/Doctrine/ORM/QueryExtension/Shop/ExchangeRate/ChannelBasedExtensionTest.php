...
final class ChannelBasedExtensionTest extends TestCase
{
    ...
    public function testAppliesConditionsToCollectionForShopApiSection(): void
    {
        ...
        $this->sectionProviderMock->expects($this->once())->method('getSection')->willReturn($shopApiSectionMock);
        $channelMock->expects($this->once())->method('getBaseCurrency')->willReturn($currencyMock);
        $queryNameGeneratorMock->expects($this->once())->method('generateParameterName')->with(':currency')->willReturn(':currency');
        ...
        $exprMock->expects($this->once())->method('orX')->with($exprComparisonMock, $exprComparisonMock)->willReturn($exprOrxMock);
        $queryBuilderMock->expects($this->once())->method('andWhere')->with($exprOrxMock)->willReturn($queryBuilderMock);
        $queryBuilderMock->expects($this->once())->method('setParameter')->with(':currency', $currencyMock)->willReturn($queryBuilderMock);
