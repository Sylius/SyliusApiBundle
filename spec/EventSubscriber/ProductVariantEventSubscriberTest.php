...
final class ProductVariantEventSubscriberTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->productVariantEventSubscriber = new ProductVariantEventSubscriber($this->eventBusMock);
    }
    ...
    public function testDispatchesProductVariantCreatedAfterCreatingProductVariant(): void
    {
        ...
        $variantMock->expects($this->once())->method('getCode')->willReturn('MUG');
        $message = new ProductVariantCreated('MUG');
        $this->eventBusMock->expects($this->once())->method('dispatch')->with($message)->willReturn(new Envelope($message));
    ...
    public function testDoesNothingAfterWritingOtherEntity(): void
    {
        ...
        $requestMock = $this->createMock(Request::class);
        $requestMock->expects($this->once())->method('getMethod')->willReturn(Request::METHOD_PUT);
        $this->eventBusMock->expects($this->never())->method('dispatch');
        $this->productVariantEventSubscriber->postWrite(new ViewEvent(
            $kernelMock,
            $requestMock,
            HttpKernelInterface::MAIN_REQUEST,
